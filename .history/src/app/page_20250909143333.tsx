...
export default function WenoveLanding() {
  ...
  useEffect(() => {
    console.log("[v0] Carousel useEffect running, current slide:", currentSlide)
    const interval = setInterval(() => {
      setCurrentSlide((prev) => {
        const nextSlide = (prev + 1) % heroImages.length
        console.log("[v0] Auto-advancing from slide", prev, "to slide", nextSlide)
        return nextSlide
      })
  ...
  const handleSlideChange = (index: number) => {
    console.log("[v0] Manual slide change to:", index)
  ...
  <header className="absolute top-0 left-0 w-full h-24 z-10 flex items-center justify-between px-6">
    ...
    {/* Nav + Background Rectangle */}
    <div className="absolute top-[30px] left-1/2 -translate-x-1/2 flex justify-center items-center w-full max-w-[500px]">
      
      {/* Retângulo atrás da nav */}
      <div className="absolute top-[25px] w-[100%] max-w-[500px] h-[50px] rounded-[30px] bg-[#CDBBA7]/30 shadow-[0_4px_4px_rgba(0,0,0,0.25)] -translate-y-1/2" />

      {/* Navegação */}
      <nav className="top-[10px] flex items-center gap-10 text-white relative">
    ...
    <div>
      ...
      {/* Hero Section */}
      <section className="relative min-h-screen flex items-center justify-center text-center px-6 transition-all duration-1000 ease-in-out"
        style={{
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-7xl text-[#ffffff] mb-6 leading-tight"
          style={{fontFamily: "Futura, sans-serif"}}
          >
            Renovar é mais do que vestir
            <br />
            diferente.<span className="text-[#ffcc00]"> É pensar diferente.</span>
          </h1>

          <p className="text-lg md:text-[25px] font-dosis text-[#ffffff] mb-8 max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-10 justify-center mb-12">
            <Button className="bg-[#8f3332] text-[#ffffff] hover:bg-[#8b3130] px-9 py-6 font-dosis text-lg font-medium">
              Como Funciona
            </Button>
            <Button className="bg-[#88a51d] text-[#ffffff] hover:bg-[#708943] px-7 py-6 font-dosis text-lg font-medium">
              Faça seu Cadastro
            </Button>
          </div>
            ...
            {heroImages.map((_, index) => (
              <button
                ...
                className={`w-4 h-4 rounded-full transition-all duration-300 border-2 ${
                  index === currentSlide
                    ? "bg-[#ffcc00] border-[#ffcc00] scale-110"
                    : "bg-transparent border-[#ffffff] hover:bg-[#ffffff] hover:bg-opacity-50"
                }`}
