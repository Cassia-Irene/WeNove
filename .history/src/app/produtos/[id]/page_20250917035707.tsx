...
// Função para buscar dados do produto (síncrona)
...
// ⚠️ Page Assíncrona (Server Component)
export default async function ProductPage({
  params,
}: {
  params: { id: string }
}) {
  // Aqui Next.js espera async, mesmo que getProductData seja síncrona
  const product = getProductData(Number(params.id))

  if (!product) {
    return <div className="min-h-screen flex items-center justify-center text-red-600 font-bold">Produto não encontrado</div>
  }
