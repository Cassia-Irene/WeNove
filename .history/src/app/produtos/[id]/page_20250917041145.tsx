...
// Torne a função getProductData assíncrona ou, se ela já for, use await
const getProductData = async (id: number) => {
  const baseProduct = products.find((p) => p.id === id)
  if (!baseProduct) return null
  
  // Simula uma busca assíncrona (opcional, dependendo de como a função real opera)
...
export default async function ProductPage({ params }: { params: { id: string } }) {
  // AQUI É A MUDANÇA: Use `await` para aguardar a Promise retornada por getProductData
