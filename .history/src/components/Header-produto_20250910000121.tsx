...
export default function Header_Produto() {
  return (
      ...
      <header className="absolute top-0 left-0 w-full h-24 z-10 flex items-center justify-between px-6">
        ...
        {/* Right side */}
            <div className="absolute top-[30px] right-[30px] flex items-center space-x-8">
                <div className="relative">
                <ShoppingCart className="h-6 w-7 text-[#5e4f45]" />
                <span className="absolute -top-2 -right-2 bg-[#8f3332] text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                    0
                </span>
                </div>
                <div className="w-12 h-12 bg-[#88a51d] rounded-full flex items-center justify-center">
