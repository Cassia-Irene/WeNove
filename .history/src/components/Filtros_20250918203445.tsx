      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        {(Object.keys(filterOptions) as (keyof typeof filterOptions)[]).map((filter) => (
          <div key={filter} className="flex flex-col items-center relative">
            ...
            {activeFilter === filter && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 p-4 bg-[transparent] rounded w-56 z-10">
                {filterOptions[filter].map((option) => (
                  <label
                    key={option}
                    className="flex items-center justify-center gap-2 cursor-pointer font-dosis py-1"
                  >
                    <input
                      type="checkbox"
...
<div className="flex w-full gap-6">
      ...
      <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            key={i}
            className="h-40 bg-[#d9d9d9] rounded-lg flex items-center justify-center font-dosis"
          >
            Produto {i + 1}
          </div>
        ))}
